  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      order.orderItems.forEach((item) => {
        ...
        const price = isCancelledOrReturned ? 0 : (item.finalPrice || item.discountPrice || item.basePrice);
        const amount = isCancelledOrReturned ? 0 : price * item.quantity;

        const nameLines = doc.widthOfString(item.name) > 260 ?
          doc.heightOfString(item.name, { width: 260 }) / 12 : 1;

        doc.text(item.name, itemCodeX, y, { width: 260 });
